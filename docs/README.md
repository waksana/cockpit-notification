...
**Current source prepares 0.1.18 for the SDK build migration; uniform tool-free reply admission, independent menus and incremental synchronization are unchanged.**
配套宿主要求 shared-surfaces v1，精确支持 SHA 见[安装指南](installation.md)；
The target v0.1.18 assets require actual Release publication; source preparation is not publication.
具体源码覆盖和平台限制见实现说明。
...
| [Release notes](release-notes.md) | Source changes | 0.1.18 SDK build identity, unchanged notification behavior and device limits |
| [构建与安装](installation.md) | 工程契约 | 正式 SDK 包与认证、锁定依赖、打包、独立宿主配对和运行配置 |
