...
**当前源码 0.1.14 沿用明确 final 直接入账；以新安装版本交付共享 UI，独立菜单及增量同步保持不变。**
配套宿主要求 shared-surfaces v1，精确支持 SHA 见[安装指南](installation.md)；
目标 v0.1.14 资产须待 Release 实际发布，不以源码提交或功能确认代替发布。
具体源码覆盖和平台限制见实现说明。
...
| [发行说明](release-notes.md) | 开发变更 | 0.1.14 共享 UI 安装身份、保留的通知行为与旧宿主错误限制 |
