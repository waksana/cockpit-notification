...
**当前源码 0.1.16 保留未读计数几何，并在非 Linux 平台先报告明确的不支持错误；独立菜单及增量同步保持不变。** 当前源码的回复判定改为所有模型统一的无工具回复直接入账（不看 phase）。
配套宿主要求 shared-surfaces v1，精确支持 SHA 见[安装指南](installation.md)；
目标 v0.1.16 资产须待 Release 实际发布，不以源码提交或功能确认代替发布。
具体源码覆盖和平台限制见实现说明。
...
| [发行说明](release-notes.md) | 开发变更 | 0.1.16 平台边界提示、保留的通知行为与旧宿主错误限制 |
