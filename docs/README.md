...
**当前源码 0.1.5 使用独立菜单项注册，保留增量同步与精简通知界面的正式功能。**
GitHub Release 当前仍为 0.1.0，见对应 tag；固定源码构建安装与正式 Release 资产分别记录，不以功能确认代替发布。
具体源码覆盖和平台限制见实现说明。
...
| [产品要求](requirements.md) | 已确认要求 | 后端集合与 SSE 核心流程、U、计数范围、阅读后清除、红线几何与跨端一致性 |
...
| [发行说明](release-notes.md) | 开发变更 | 0.1.5 菜单注册、通知点击、推送兼容、增量同步与已知限制 |
