...
**当前源码 0.1.11 修复回复证据超时，按数量保护内存；未读展示、独立菜单及增量同步保持不变。**
配套宿主为 Cockpit 0.2.4；目标 v0.1.11 资产须待 Release 实际发布，不以源码提交或功能确认代替发布。
具体源码覆盖和平台限制见实现说明。
...
| [发行说明](release-notes.md) | 开发变更 | 0.1.11 回复证据生命周期、保留的通知行为与旧宿主错误限制 |
