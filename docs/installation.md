...
**0.1.3 增量同步与精简入口为正式功能，可以从固定源码构建安装。** 需要配套源码提供的通用模块事件，
...
GitHub 尚未发布 0.1.3 Release 资产；固定源码产包不等于已发布 Release，不使用旧包或源码 ZIP 冒充新安装包。
...
```sh
pnpm package
pnpm verify:package module-output/cockpit-notification-0.1.3.tgz
```
...
node --import ./apps/server/node_modules/tsx/dist/loader.mjs \
  ...
  /absolute/path/cockpit-notification-0.1.3.tgz --trust-local-code --enable
```
