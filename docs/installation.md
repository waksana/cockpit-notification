...
**当前源码 0.1.8 配套 Cockpit 0.2.4，使用暖黄色圆角未读底色与缓慢淡出。** 增量同步、会话徽标与精简入口保持不变，本次无需宿主改动。
...
0.1.8 的目标正式安装包为 [v0.1.8 Release](https://github.com/waksana/cockpit-notification/releases/tag/v0.1.8)
中的 `cockpit-notification-0.1.8.tgz` 与同名 `.sha256`；仅在该 Release 及资产实际发布后下载，
并执行 `sha256sum -c cockpit-notification-0.1.8.tgz.sha256`。本说明不宣称已发布或部署。
...
```sh
pnpm package
pnpm verify:package module-output/cockpit-notification-0.1.8.tgz
```
...
node --import ./apps/server/node_modules/tsx/dist/loader.mjs \
  ...
  /absolute/path/cockpit-notification-0.1.8.tgz --trust-local-code --enable
```
