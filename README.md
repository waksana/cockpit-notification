...
**当前源码版本为 0.1.7，配套 Cockpit 0.2.4，通知开关通过宿主独立菜单注册接入。**
0.1.7 将回复未读标记改为正文内的主题感知浅色底色；发布与安装状态仍以实际结果为准。
增量同步与精简通知界面是正式功能。目标安装资产须待
[v0.1.7 Release](https://github.com/waksana/cockpit-notification/releases/tag/v0.1.7) 实际发布后使用，
...
- 已完成且已知未读的主 Agent 回复，在实际正文内使用主题强调色的 8% 透明混合底色，
  不增加 padding、margin 或 DOM 包装，不改变布局。未读移除后底色用 200ms 淡出，
