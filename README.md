...
**当前源码版本为 0.1.8，配套 Cockpit 0.2.4，通知开关通过宿主独立菜单注册接入。**
0.1.8 使用暖黄色圆角未读底色，已读后以 2.4 秒淡出；发布与安装状态仍以实际结果为准。
增量同步与精简通知界面是正式功能。目标安装资产须待
[v0.1.8 Release](https://github.com/waksana/cockpit-notification/releases/tag/v0.1.8) 实际发布后使用，
...
- 已完成且已知未读的主 Agent 回复，在实际正文内使用混合主题强调色的暖黄色圆角底色，
  以 18% 不透明度叠加，不增加 padding、margin 或 DOM 包装，不改变布局。未读移除后底色用 2400ms 淡出，
